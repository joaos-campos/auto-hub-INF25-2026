...
-- Table T_SERVICO
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `DB_AAH`.`T_AAH_SERVICO` 
(
  ...
  INDEX `IDX_FK_ATENDIMENTO_SERVICO` (`id_atendimento` ASC),
  CONSTRAINT `FK_ATENDIMENTO_SERVICO`
    FOREIGN KEY (`id_atendimento`)
    REFERENCES `DB_AAH`.`T_AAH_ATENDIMENTO` (`id_atendimento`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `CK_SERVICO_VL_SERVICO` CHECK (`vl_servico` > 0 AND `vl_servico` <= 9999999.99),
  CONSTRAINT `CK_SERVICO_VL_HORAS_GASTAS` CHECK (`vl_horas_gastas` > 0 AND `vl_horas_gastas` < 10000)
)
ENGINE = InnoDB;
